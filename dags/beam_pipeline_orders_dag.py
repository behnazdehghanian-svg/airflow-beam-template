...
DAG_ID           = env_replace(config["dag"].get("orders_dag_id", "beam_pipeline_orders"))
...
PYTHON_SCRIPT    = env_replace(config["beam"].get("orders_python_file", "/home/airflow/gcs/dags/beam_pipeline_orders.py"))
...
ORDERS_TABLE = {
    ...
    "schema":      "order_id:STRING,customer_id:STRING,amount:FLOAT,order_date:STRING",
}
...
with DAG(
    dag_id=DAG_ID,
    description="Beam ETL pipeline: load orders CSV into BigQuery.",
    ...
    load_orders = BeamRunPythonPipelineOperator(
        task_id=f"beam_load_{ORDERS_TABLE['bq_table']}",
        runner=RUNNER,
        ...
        pipeline_options={
            "project":          PROJECT_ID,
            "input_path":       f"gs://{SOURCE_BUCKET}/{SOURCE_FOLDER}/{ORDERS_TABLE['source_file']}",
            "bq_dataset":       ORDERS_TABLE["bq_dataset"],
            "bq_table":         ORDERS_TABLE["bq_table"],
            "bq_schema":        ORDERS_TABLE["schema"],
